...
class AudioSessionManager {
    ...
    func configureForVoiceInput() {
        ...
        do {
            ...
            if let availableInputs = audioSession.availableInputs,
               let builtInMic = availableInputs.first(where: {
               ...
               })
            {
                ...
                if let dataSource = frontSource {
                    ...
                    if let supportedPatterns = dataSource
                        .supportedPolarPatterns,
                        let cardioid = supportedPatterns.first(where: {
                            pattern in
            ...
            print(
                "✅ [AudioSession] Configured: 16kHz, Cardioid, VAD/ASR optimized"
            )

        } catch {
            // Don't crash in production, just log the error
            print(
                "⚠️ [AudioSession] Configuration failed: \(error.localizedDescription)"
            )
    ...
    func configureForHighQualityRecording() {
        ...
        do {
            ...
            print(
                "⚠️ [AudioSession] HQ recording config failed: \(error.localizedDescription)"
            )
        }
    }
    ...
    func configureForHighQualityASR() {
        ...
        do {
            ...
            print(
                "⚠️ [AudioSession] HQ ASR config failed: \(error.localizedDescription)"
            )
        }
    }
    ...
    func setupRouteChangeObserver(
        onChange: @escaping (AVAudioSession.RouteChangeReason) -> Void
    ) {
    ...
    func logAudioRoute() {
        ...
        print(
            "🎙️ [AudioSession] Route: \(audioSession.currentRoute.inputs.first?.portName ?? "Unknown")"
        )
    }
}
