-- Create a function to increment post views atomically
-- This function should be executed in Supabase SQL Editor

CREATE OR REPLACE FUNCTION increment_post_views(post_id UUID)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  UPDATE public."7verse_posts"
  SET views = COALESCE(views, 0) + 1
  WHERE id = post_id;
END;
$$;

-- Grant execute permission to authenticated and anon roles
GRANT EXECUTE ON FUNCTION increment_post_views(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION increment_post_views(UUID) TO anon;
GRANT EXECUTE ON FUNCTION increment_post_views(UUID) TO service_role;

-- Function to toggle post like
-- This function increments or decrements the likes_count atomically
CREATE OR REPLACE FUNCTION toggle_post_like(post_id UUID, is_liked BOOLEAN)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  IF is_liked THEN
    -- User liked the post, increment count
    UPDATE public."7verse_posts"
    SET likes_count = COALESCE(likes_count, 0) + 1
    WHERE id = post_id;
  ELSE
    -- User unliked the post, decrement count (but not below 0)
    UPDATE public."7verse_posts"
    SET likes_count = GREATEST(COALESCE(likes_count, 0) - 1, 0)
    WHERE id = post_id;
  END IF;
END;
$$;

